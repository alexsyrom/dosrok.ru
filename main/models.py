...
class Student(models.Model):
    lastname = models.CharField('Фамилия', max_length=50, blank=True, default='')
    firstname = models.CharField('Имя', max_length=50, blank=True, default='')
    group = models.CharField('Группа', max_length=6, blank=True, default='')
    ...
    PRS = models.PositiveIntegerField("БРС", default=0, blank=True)
    test_mark = models.PositiveIntegerField("Семестровая", default=0, blank=True)
    def __str__(self):
    ...
    def clean(self):
        ...
        if not self.lastname and not self.firstname and not self.group:
            raise ValidationError("Хотя бы одно из полей 'Фамилия', 'Имя', 'Группа' должно быть заполнено")
    class Meta:
...
def update_rating(sender, instance, action, reverse, model, pk_set, **kwargs):
    """
    I'm really sorry, but Django doesn't send remove signal. It even doesn't do remove opearions, it clears all and after it adds.
